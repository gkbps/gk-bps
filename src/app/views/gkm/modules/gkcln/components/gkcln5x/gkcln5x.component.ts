...
import { getGkClientDashboardsAction } from '../../../../../../ngrx/gkClient/gkClients.actions';
/**/
...
export class GkCln5xComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store.dispatch(getGkClientDashboardsAction('', '{}', 0, 10));
    this.gkClientDashboards = store.pipe(select('gkClientDashboards'));
  }
  ...
  doSomething(event) {
    ...
    this.store.dispatch(getGkClientDashboardsAction(
      event.filter,
      event.sort,
      event.first,
      event.rows
