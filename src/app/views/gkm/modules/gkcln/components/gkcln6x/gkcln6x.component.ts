...
import { getGkClientReportsSummaryAction } from '../../../../../../ngrx/gkClient/gkClients.actions';
/**/
...
export class GkCln6xComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store.dispatch(getGkClientReportsSummaryAction('', '{}', 0, 10));
    this.gkClientDashboards = store.pipe(select('gkClientReportsSummary'));
  }
  ...
  doSomething(event) {
    ...
    this.store.dispatch(getGkClientReportsSummaryAction(
      event.filter,
      event.sort,
      event.first,
      event.rows
